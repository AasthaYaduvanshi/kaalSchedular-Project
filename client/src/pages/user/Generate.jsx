...
export default function Generate() {
  ...
  function getDatesInInterval(roomDateRanges) {
    ...
    for (var j = 0; j < datesByInterval.length; j++) {
      let timetable = GenerateFunction(
        courses,
        selectedRooms,
        datesByInterval[j]
      )
  ...
  function GenerateFunction(courses, selectedRooms, days) {
    ...
    courses.forEach((course) => {
      course.subjects.forEach((subject) => {
        ...
        days.forEach((day) => {
          teacherAvailability[subject.teacher][day] = new Array(
      ...
      shuffledCourses.forEach((course) => {
        ...
        for (let i = 0; i < course.subjects.length; i++) {
          ...
          while (teacherAvailability[teacher][day][i]) {
            const otherTeachers = Object.keys(teacherAvailability).filter(
              (t) => t !== teacher
            )
            teacher =
              otherTeachers[Math.floor(Math.random() * otherTeachers.length)]
            tries++
      ...
      Object.entries(course).forEach(([day, lectures]) => {
        ...
        for (let i = 0; i < numLectures; i++) {
          ...
          course[day][i].timeSlot = {
            start: `${startTime < 12 ? startTime : startTime - 12}:00 ${
              startTime < 12 ? "AM" : "PM"
            }`,
            end: `${endTime < 12 ? endTime : endTime - 12}:00 ${
              endTime < 12 ? "AM" : "PM"
            }`,
          }
          startTime += 1
  ...
  const handleDeleteCourse = async (course) => {
    try {
      ...
      const response = await api.delete(
        `/api/user/delete-course?id=${course._id}`
      )
                  ...
                  {[...Array(maxLectures).keys()].map((hour, i) => (
                    <TableRow key={hour}>
                      ...
                      <BasePopup id={id} open={open} anchor={anchor}>
                        <PopupBody>
                          <Typography>Add lunch break</Typography>
                          <Box
                            flexDirection={"row"}
                            alignItems={"center"}
                          ...
                          >
                            <IconButton
                              onClick={() =>
                                handleMoveLunchBreak("up", lunchBreakPosition)
                              }
                            >
                              <IconArrowBigUpLine />
                            </IconButton>
                            <IconButton
                              onClick={() =>
                                handleMoveLunchBreak("down", lunchBreakPosition)
                              }
                            >
                              <IconArrowBigDownLine />
                            </IconButton>
                      ...
                      {Object.keys(timetable).map((day) => {
                        const lecturesForDayAndTimeSlot = timetable[day].filter(
                          (lecture) =>
                            parseInt(lecture.timeSlot.start) ===
                              (9 + hour) % 12 &&
                            parseInt(lecture.timeSlot.end) === (10 + hour) % 12
                        )
                        return (
                          <TableCell key={day}>
              ...
              >
                ...
                >
                  <FormControl style={{ minWidth: 200, marginRight: 10 }}>
                    <InputLabel id={`room-select-label-${i}`}>
                      Select Room
                    </InputLabel>
                    <Select
                    ...
                    >
                      {rooms.map((room, j) => (
                        <MenuItem key={j} value={room.roomNumber}>
                          {room.roomNumber}
                        </MenuItem>
                      ))}
                    </Select>
          ...
          <Box sx={{ mb: 2, mt: 6 }}>
            <div>
              <SubmitButton
                variant="contained"
                onClick={() => {
                  getDatesInInterval(roomDateRanges)
                }}
              >
                Generate
              </SubmitButton>

...
import { IconCertificate, IconTableImport, IconSchool, IconChalkboard, IconBaguette } from '@tabler/icons'
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, Button, FormControl, Select } from '@mui/material';
...
export default function FullWidthTabs() {
  ...
  const filteredTeachers = teachers.filter(user => user.name.toLowerCase().includes(filter.toLowerCase()));

  const fetchData = async (pageno, rowsPerPagesent) => {
    const response = await api.get(`http://localhost:4000/api/user/fetch-teachers?page=${pageno + 1}&pageSize=${rowsPerPagesent}`);
  ...
  React.useEffect(() => {
    const fetchDataClasses = async () => {
      const response = await api.get(`http://localhost:4000/api/user/fetch-rooms?page=${pageforclass + 1}&pageSize=${rowsPerPageforclass}`);
      const data = response.data;
      console.log(data)
    ...
    fetchDataClasses();
  }, [pageforclass, rowsPerPageforclass]);
    ...
    <Box sx={{ width: '100%', bgcolor: 'background.paper' }}>
      ...
      <TabPanel value={value} index={0} dir={theme.direction}>
        ...
        >
          ...
          <Item1 sx={{ gridRow: '1', gridColumn: '4 / 5' }}>
            <SubmitButton onClick={handleAddRoomOpen}>
              Add Room
            </SubmitButton>
          </Item1>
        </Box>
      ...
      <TabPanel value={value} index={1} dir={theme.direction}>
        ...
        >
          ...
          <Item1 sx={{ gridRow: '1', gridColumn: '4 / 5' }}>
            <SubmitButton onClick={handleAddUserOpen}>
              Add teacher
            </SubmitButton>
          </Item1>
        </Box>
        ...
        >
          <Box sx={style}>
            <AddTeachers getConfirmation={teacherCreatedConfirmation} />
          </Box>
        </Modal>
        <TableContainer component={Paper}>
          <Table>
      ...
      <TabPanel value={value} index={2} dir={theme.direction}>
        <Box display="flex" justifyContent="center" p={2}>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={6}>
              <img src={graduated} alt="Your Image" style={{ maxWidth: '80%', height: 'auto', display: 'block', marginLeft: 'auto' }} />
            </Grid>
            <Grid item xs={6}>
              <AddCourse />
            </Grid>

...
export default function AddCourse() {
  ...
  const { control, handleSubmit, formState: { errors }, } = useForm({ resolver: zodResolver(schema), mode: 'onChange', defaultValues });
  const handleNext = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const teachersResponse = await api.get('api/user/fetch-all-teachers');
        setTeachers(teachersResponse.data.teachers);

        const filteredCoursesResponse = await api.get('api/user/get-filtered-courses');
  ...
  const [subjectNames, setSubjectNames] = React.useState(Array(numberofSubjects).fill(''));
  ...
  const CheckTeacherAvailability = (numberOfSubjects) => {
    const teachersWithOneSubject = teachers.filter(teacher => !teacher.isAssignedToTwoSubjects);

    if (teachersWithOneSubject.length < numberOfSubjects) {
      setLessTeachersAvailable(true)
      toast.error(`Not enough teachers available for ${numberOfSubjects} subjects.`);
    }
  };
  ...
  const steps = [
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormControl variant="standard" sx={{ m: 1, minWidth: 360 }}>
                ...
                {errors.course && <span style={{ fontSize: "0.6rem", color: "red" }}>{errors.course.message}</span>}
              </FormControl>
            )}
          />
    ...
    },
    {
      ...
      description:
        'Add the number of subjects for the current course',
      formComponent: (
        <Box p={2}>
          <FormControl variant="standard" sx={{ m: 1, minWidth: 260 }}>
            <InputLabel id="demo-simple-select-filled-label">Number of subjects</InputLabel>
            <Select labelId="demo-simple-select-filled-label" value={numberofSubjects} onChange={handleNoOfSubjects} fullWidth>
              {[...Array(6)].map((_, index) => (
                <MenuItem key={index + 2} value={index + 2}>{index + 2}</MenuItem>
              ))}
            </Select>
          ...
          <Box sx={{ mb: 2 }}>
            <div>
              ...
              <Button style={{ color: '#FFC801' }} onClick={handleBack} sx={{ mt: 1, mr: 1 }}>
                Back
              </Button>
            </div>
          </Box>
    ...
    {
      label: 'Create an ad',
      description: `Select teachers for subjects ${subjectNames.map(name => (' ' + name))} orderwise`,
      formComponent: (
        <Box p={2}>
          <MultiSelect sendTheTeachers={getTheTeachersAndMapWithSubjects} noOfSubjects={numberofSubjects} teachers={teachers} />
          {LessTeachersAvailable ? <span style={{ fontSize: "0.6rem", color: "red" }}>Less teachers available {`(Add new teachers)`}</span> : null}
          <div>
            {
              TeachsersSelected && TeachsersSelected.length === numberofSubjects ? (
                <Button
                  variant="contained"
                  type="submit"
                  style={{ background: '#FFC801' }}
